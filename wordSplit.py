...
def _rect_digital(img):
    ...
    if red_pos:  # 如果存在数字区域，将数字区域作为结尾
        redy0, redy1 = red_pos
        w = redy1
    #     digital_rect = digital_rect[0:h, 0:w]  # 重新取数字区
    # else:
...
def img_to_words(img, show=False):
    ...
    digital_rect = digital1[y2:y2+h2, x2:x2+w2]
    x = x1 + x2
    y = y1 + y2
    w = w2
    h = h2
    ...
    #     cv2.imshow('digital rect', digital_rect)
    # 步骤3：图像预处理
    closed = pre_process(digital_rect, show)
    # 步骤4：进行列分割，分割的图像基于数字区
    ...
    for i in range(0, len(xWords)):
     ...
     # 步骤6：对切割结果过滤，去除不可能是数字的部分：取字符高度的中位数，去除高度小于高度中位数1/3的字符
...
if __name__ == '__main__':
    img = cv2.imread('./train-source1/4.jpg')
